...
bar = function anonymous (doubleArray) {
  var i;
  var canMult = true;
  for (i = 0; i < doubleArray.length && canMult == true; i++) {
...
function parseGit(logArray) {
  ...
  for (i = 0; i < logArray.length; i++) {
    d = new Date(logArray[i].slice(logArray[i].indexOf(' '), logArray[i].indexOf('\"')));
    GitLog[i] = {
      hash: logArray[i].slice(0, logArray[i].indexOf(' ')),
      message: logArray[i].slice(logArray[i].indexOf('\"') + 1, logArray[i].lastIndexOf('\"')),
      date: d
    }
  }
  return GitLog;
